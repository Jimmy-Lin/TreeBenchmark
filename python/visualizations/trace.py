...
def replot(i):
    ...
    for node in G:
        # y_group = round((sum( int(c) for c in node ) - y_min) / max(1, y_step))
        # x.append(xs[y_group].index(int(node)) - 0.5 * len(xs[y_group]))
        x.append(int(node))
    ...
    # print(renderer.node_renderer.data_source.data)
    # print(renderer.edge_renderer.data_source.data)bo
    ...
    # renderer.node_renderer.selection_glyph = Circle(size=10, fill_color=Spectral4[2])
    # renderer.node_renderer.hover_glyph = Circle(size=10, fill_color=Spectral4[1])

    if m > 0:
        renderer.edge_renderer.glyph = MultiLine(line_color="edge_color", line_alpha=0.4, line_width=2)
        # renderer.edge_renderer.selection_glyph = MultiLine(line_color=Spectral4[2], line_width=3)
        # renderer.edge_renderer.hover_glyph = MultiLine(line_color=Spectral4[1], line_width=3)
